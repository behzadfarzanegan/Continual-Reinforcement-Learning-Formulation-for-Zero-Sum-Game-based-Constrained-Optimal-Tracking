...
for k = 2:10000
    ...
    %     BF1 = log((-0.2*0.2)/((0.2-e(1,k-1))*(-.2-e(1,k-1))))*Barrier;
%     BF2 = log((-0.3*0.3)/((0.3-e(2,k-1))*(-.3-e(2,k-1))))*Barrier;
% 
%     Yk_1 = e(:,k-1)'*Q*e(:,k-1)+u(:,k-1)'*R*u(:,k-1)- gamma2*w(:,k-1)'*w(:,k-1)+ 0*Z(:,k)'*Q*Z(:,k)+100000*BF1 +100*BF2;

    BF1 = log((-0.3*0.3)/((0.3-e(1,k-1))*(-.3-e(1,k-1))))*Barrier;
    BF2 = log((-0.3*0.3)/((0.3-e(1,k-1))*(-.3-e(1,k-1))))*Barrier;

    Yk_1 = e(:,k-1)'*Q*e(:,k-1)+u(:,k-1)'*R*u(:,k-1)- gamma2*w(:,k-1)'*w(:,k-1)+ 0*Z(:,k)'*Q*Z(:,k)+1000*BF1 +10000*BF2;
    ...
    cost_ins(k)=e(:,k-1)'*Q*e(:,k-1)+u(:,k-1)'*R*u(:,k-1)- gamma2*w(:,k-1)'*w(:,k-1);
...
%                 if LL ==1
        if 3000 <= k && k < 6000
            ...
            temp_1 =temp_1-aj*(delta_xK*EJK)/(delta_xK'*delta_xK+1)-PE_C*aj*M1*P1-1*aj*Fisher1*(temp_1-W_critic(:,:,2999))*EWC;
        elseif 6000<=k
            temp_1 =temp_1-aj*(delta_xK*EJK)/(delta_xK'*delta_xK+1)-PE_C*aj*M1*P1-1*aj*Fisher2*(temp_1-W_critic(:,:,5999))*EWC;
        else
...
%         disturbance(k)=0.1*exp(-0.1*(k-tdist)*1)*dist_on;
    ...
    [e(:,k+1),r(:,k+1),Gx,Z(:,k+1)] = NLinear_sys_NL_gamma_bah(x(:,k),u(:,k),disturbance(k),H,r(:,k),k,Z(:,k),e(:,k));
    x(:,k+1)=e(:,k+1)+r(:,k+1);
    X(:,k+1) = [e(:,k+1);r(:,k+1);1];
    AugX(:,k+1) = [e(:,k+1);r(:,k+1);Z(:,k+1)];
