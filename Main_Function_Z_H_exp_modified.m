...
for k = 2:3000
    ...
    [e(:,k),r(:,k),Gx,Z(:,k)] = NLinear_sys_NL_gamma_bah(x(:,k-1),u(:,k-1),disturbance(k-1),H,r(:,k-1),k,Z(:,k-1),e(:,k-1));

    B1 = log((-0.3*0.3)/((0.3-e(1,k))*(-.3-e(1,k))))*Barrier;
    B2 = log((-0.3*0.3)/((0.3-e(2,k))*(-.3-e(2,k))))*Barrier;
    BF(k)=B1+B2;
    if   BF(k)-BF(k-1)>= -0.9*BF(k)
        [e(:,k),r(:,k),Gx,Z(:,k)] = NLinear_sys_NL_gamma_bah(x(:,k-1),u(:,k-1),disturbance(k-1),H,r(:,k-1),k,Z(:,k-1),e(:,k-1));

    else
        PHI = BF(k)-BF(k-1) + 0.9*BF(k);
        PHI0 = [-(200*e(1,k-1))/(100*e(1,k-1)^2 - 9) -(200*e(2,k-1))/(100*e(2,k-1)^2 - 9)  0 0 0 0]*Gx;
        u_qp=PHI*PHI0'/(PHI0*PHI0');
        u(:,k-1)=u(:,k-1)-u_qp;
        [e(:,k),r(:,k),Gx,Z(:,k)] = NLinear_sys_NL_gamma_bah(x(:,k-1),u(:,k-1),disturbance(k-1),H,r(:,k-1),k,Z(:,k-1),e(:,k-1));
    end
...
%     end
    ...
    BF1 = log((-0.3*0.3)/((0.3-e(1,k))*(-.3-e(1,k))))*Barrier;
    BF2 = log((-0.3*0.3)/((0.3-e(1,k))*(-.3-e(1,k))))*Barrier;



    Yk_1 = e(:,k)'*Q*e(:,k)+u(:,k)'*R*u(:,k)- gamma2*w(:,k)'*w(:,k)+ 0*Z(:,k)'*Q*Z(:,k)+1000*BF1 +10000*BF2;
    delta_x = gamma*Critic_NL_gamma_bah(AugX(:,k))-Critic_NL_gamma_bah(AugX(:,k-1));
    EJK = Yk_1+W_critic(:,:,k)'*delta_x;
    YK = X(:,k)'*QN*X(:,k)+u(:,k-1)'*R*u(:,k);
    ...
    cost_ins(k)=e(:,k)'*Q*e(:,k)+u(:,k)'*R*u(:,k)- gamma2*w(:,k)'*w(:,k);
...
% 
    ...
    if k>tdist
%         disturbance(k)=0.1*exp(-0.1*(k-tdist)*1)*dist_on;
        disturbance(k)=0.1*exp(-.01*(k-tdist)*1)*dist_on*sin(1*(k-tdist)*1);
    end


    u_tilda = W_actor(:,:,k)'*Actor_NL_gamma_bah(X(:,k),v_actor(:,:,1),Neuron_Num_a)+gamma*0.5*inv(R)*Gx'*Dsigdx_NL_gamma_bah(AugX(:,k))'*W_critic(:,:,k+1);
    ...
    u(:,k+1) = W_actor(:,:,k+1)'*Actor_NL_gamma_bah(X(:,k),v_actor(:,:,1),Neuron_Num_a);

    wopt(:,k)=inv(gamma2)*0.5*H'*Dsigdx_NL_gamma_bah(AugX(:,k))'*W_critic(:,:,k+1);
    w_tilda = W_actor2(:,:,k)'*Actor_NL_gamma_bah(X(:,k),v_actor2(:,:,1),Neuron_Num_a2) - wopt(:,k);
    tempw = Actor_NL_gamma_bah(X(:,k),v_actor2(:,:,1),Neuron_Num_a2);
    W_actor2(:,:,k+1) = W_actor2(:,:,k)-aw*tempw*w_tilda';
    w(:,k+1) = W_actor2(:,:,k+1)'*Actor_NL_gamma_bah(X(:,k),v_actor2(:,:,1),Neuron_Num_a2);
...
plot(SS,'k');
xlabel('Sampling instants','FontWeight','b','FontSize',12);
ylabel(' Value Function','FontWeight','b','FontSize',12);
% title('Estimated Cost','FontWeight','b','FontSize',12);
grid on;box on;
set( gca, 'FontWeight', 'b','FontSize', 12 );
