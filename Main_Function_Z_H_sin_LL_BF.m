...
%     end
    ...
    BF2 = log((-0.2*0.2)/((0.2-e(2,k-1))*(-.2-e(2,k-1))))*Barrier;

    Yk_1 = e(:,k-1)'*Q*e(:,k-1)+u(:,k-1)'*R*u(:,k-1)- gamma2*w(:,k-1)'*w(:,k-1)+ 0*Z(:,k)'*Q*Z(:,k)+1000*BF1 +1000*BF2;
    ...
    cost_ins(k)=e(:,k-1)'*Q*e(:,k-1)+u(:,k-1)'*R*u(:,k-1);
    cost_com(k)=SS(k);
...
% 
    ...
    [e(:,k+1),r(:,k+1),Gx,Z(:,k+1)] = NLinear_sys_NL_gamma_bah_sin(x(:,k),u(:,k),disturbance(k),H,r(:,k),k,Z(:,k),e(:,k));
    x(:,k+1)=e(:,k+1)+r(:,k+1);
    X(:,k+1) = [e(:,k+1);r(:,k+1);1];
